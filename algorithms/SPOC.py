...
class SPOC(object):
    ...
    Parametrs
    ---------
    A:  array-like, shape: (n_clusters, n_clusters)
    ...
    std_num: float, optional, default: 3
        Influence on size of ellipsoid.

    use_bootstrap: boolean, oprional, default: False
        Whether to use bootatrap realization of SPOC.

    return_pure_nodes_indices: boolean, oprional, default: False
        if True then also returns a list of indices of selected pretenders
        to be pure nodes.
    
    use_cvxpy: boolean, oprional, default: False
        Whether to use cvxpy in order to compute Theta.
        
    use_ellipsoid: boolean, oprional, default: False
        If True then ellipsoid transformation is used to transform coordinates
        else basic method is used.

    use_convex_hull: boolean, oprional, default: False
    ...
    return_bootstrap_matrix: boolean, oprional, default: False
        If True then also returns bootstrap result.

    '''

    def __init__(self, A=None, n_clusters=None, n_repetitions=30,
                 std_num=3.0, use_bootstrap=False, use_convex_hull=False,
                 return_pure_nodes_indices = False, use_ellipsoid=False,
                 bootstrap_type='random_weights', use_cvxpy=False,
    ...
    def fit(self, **kwargs):
        ...
        if self.use_bootstrap == False:
            ...
            elif (self.return_bootstrap_matrix == True) and \
                 (self.return_pure_nodes_indices == False):
                return Theta, B, repeats
            elif (self.return_bootstrap_matrix == False) and \
                 (self.return_pure_nodes_indices == True):
                return Theta, B, J
            else:
                return Theta, B
