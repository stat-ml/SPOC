...
def generate_theta(n_nodes, n_clusters, pure_nodes_number, **kwargs):
    ...
    **kwargs:
    ...
    pure_nodes_matrix = np.array(pure_nodes_matrix).reshape((pure_nodes_number, n_clusters))

    if "pure_nodes_indices" in kwargs:
        pure_nodes_indices = kwargs["pure_nodes_indices"]
        assert len(pure_nodes_indices) == pure_nodes_number, "len of pure_nodes_indices != pure_nodes_number"
...
def generate_p(n_nodes, n_clusters, pure_nodes_number, **kwargs):
    ...
    **kwargs:
    ...
    pure_nodes_indices: array with indices of pure node rows in Theta matrix (used only if Theta is not set up)
    ...
    if "Theta" in kwargs:
        ...
        Theta = generate_theta(n_nodes, n_clusters, pure_nodes_number, **kwargs)
